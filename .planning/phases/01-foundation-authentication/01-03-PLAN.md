...
<task type="auto">
  ...
  <verify>
    ...
    1. Create a quick test script (`scripts/test-session.ts`) that imports encrypt and decrypt from session.ts, encrypts a sample payload {userId: "test", role: "ADMIN", expiresAt: new Date()}, decrypts it, and asserts the userId matches. Run with `npx tsx scripts/test-session.ts`. Delete the script after verification.
    2. Start dev server (`npm run dev &`), then:
       - `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/dashboard` should return 307 (redirect to /login)
       - `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/login` should return 200
    3. Kill dev server.
  </verify>
  <done>
    ...
    - Session encrypt/decrypt roundtrip works correctly with sample payload
