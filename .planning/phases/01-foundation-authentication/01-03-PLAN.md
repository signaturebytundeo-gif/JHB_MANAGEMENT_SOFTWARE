...
must_haves:
  truths:
    ...
    - "User can request a magic link and log in without a password"
...
<objective>
Implement the invite system for new users with pre-assigned roles, optional magic link authentication, role-specific dashboard views for all 4 roles, user management page for admins, and loading skeletons for data-heavy views.
...
Output: Working invite flow, magic link auth, 4 role-specific dashboard views, user management page, loading skeletons.
</objective>
...
<task type="auto">
  <name>Task 1: Implement invite system and magic link authentication</name>
  <files>
    src/app/actions/auth.ts
    src/app/(auth)/invite/[token]/page.tsx
    src/app/(auth)/magic-link/page.tsx
  ...
  <verify>
    ...
    npm run build
    ```
    TypeScript compiles and build succeeds. Then test:
    1. Log in as Anthony (admin)
    2. Navigate to /settings/users (will be built in Task 2, but action should work)
    3. Send invite to test@example.com with SALES_REP role
    4. Check console for invite email (dev mode without RESEND_API_KEY)
    5. Visit invite URL — should show acceptance form
    6. Set password and submit — should create account and redirect to dashboard
    7. Log out, visit /magic-link, enter anthony@jamaicahousebrand.com
    8. Check console for magic link email
    9. Visit magic link URL — should log in and redirect to dashboard
  </verify>
  <done>
    ...
    - Magic link verification endpoint: validates token, marks as used, creates session
...
<task type="auto">
  ...
  <action>
    ...
    4. Update `src/app/(dashboard)/page.tsx` — Main dashboard (AUTH-05):
       ...
       - Quick action buttons section (placeholder, not functional yet):
       ...
       - Each button: shadcn Button variant "outline", Caribbean gold border
  ...
  <verify>
    ...
    npm run build
    ```
    Build succeeds. Then test the full flow:
    1. Log in as Anthony (ADMIN) — see admin dashboard with 6 KPI cards and quick actions
    2. Navigate to Settings > Users — see user list and invite form
    3. Send invite to test@test.com as SALES_REP — toast success
    4. Log out, accept invite at /invite/{token} — create account
    5. Log in as new SALES_REP — see sales-focused dashboard (4 KPI cards, no production/inventory)
    6. Log in as Anthony again, visit /dashboard/investor — see investor dashboard with equity info
    7. Verify SALES_REP cannot access /dashboard/investor (redirect to /dashboard)
    8. Verify SALES_REP cannot access /settings (redirect or error)
    9. Check that loading skeletons appear briefly before KPI cards render
    10. Test on mobile viewport — cards stack, sidebar is overlay
  </verify>
  <done>
    - Admin sees all KPI cards (6) + all quick action buttons + full sidebar navigation
    - Manager sees operations KPI cards (6) + limited quick actions
    - Sales Rep sees sales-focused KPI cards (4) + sales quick actions
    ...
    - Quick action buttons are labeled but not yet functional (links to future pages)
  </done>
</task>
...
2. Admin navigates to Settings > Users -> sees user list, invite form, pending invites
3. Admin sends invite to new email -> invite token created, email sent (or logged)
4. New user accepts invite at /invite/{token} -> sets password, account created, redirected to dashboard
5. New Sales Rep logs in -> sees only sales-focused dashboard (4 KPIs, limited sidebar)
6. Sales Rep cannot access /settings or /dashboard/investor (blocked by role check)
7. Admin views /dashboard/investor -> sees investor dashboard with equity structure
8. User requests magic link -> token created, email sent, link works for 15 minutes
9. Used magic link cannot be reused
10. Expired invite (>7 days) shows error message
11. Loading skeletons appear in dashboard before KPI cards render
12. All pages work on mobile viewport (responsive)
</verification>

<success_criteria>
1. Invite system works end-to-end: admin sends invite -> user accepts -> account created with correct role
2. Magic link auth works: request link -> receive email -> click link -> logged in
3. All 4 roles see appropriate dashboard content (Admin, Manager, Sales Rep, Investor)
4. Access control enforced: each role can only see permitted sections
5. User management page (admin-only) shows users, invites, and invite form
6. Loading skeletons display during data loading transitions
7. All views are mobile-first responsive
8. Token expiration enforced (7 days invite, 15 min magic link)
</success_criteria>
